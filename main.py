...
if __name__ == '__main__':
	output_filename_root = output_filename
	output_filename = output_filename_root + '_ours'
	fl = CustomFL(
	...
	with open(f'{output_filename}/{output_filename}_{begin_time}.txt'.replace(':', '-'), 'wb') as f:
		pickle.dump(fl.debug_log, f)

	output_filename = output_filename_root + '_fl'
	clustering_on=0
	fl = CustomFL(
		n_iter=n_iter, n_epochs=n_epochs, poison_starts_at_iter=poison_starts_at_iter, 
		validation_starts_at_iter=validation_starts_at_iter, 
